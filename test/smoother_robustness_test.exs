defmodule BstsNx.SmootherRobustnessTest do
  ...
  test "simulate_with_key degrades gracefully when terminal covariance is not positive-definite" do
    bad_cov = Nx.tensor([[1.0, 2.0], [2.0, 1.0]])
    smoothed = [{Nx.tensor([0.0, 0.0]), bad_cov}]
    filtered = smoothed
    predicted = smoothed
    noise = [Nx.tensor([0.0, 0.0])]
    ...
    log =
      capture_log(fn ->
        ...
        assert Nx.shape(hd(states)) == {2}
      end)

    assert log =~ "terminal covariance not positive-definite"
  end

  test "simulate_with_key degrades gracefully when conditional covariance is not positive-definite" do
    ...
    noise = [Nx.tensor([0.0, 0.0]), Nx.tensor([0.0, 0.0])]
