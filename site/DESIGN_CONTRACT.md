...
2. `<.section_heading eyebrow="..." title="...">` — eyebrow is the question or
   chapter label; body is 2–4 sentences of scene-setting prose.
...
`BstsSiteWeb.Charts`:
- `<.line_figure id= series=[%{points: [float], x_offset: 0, color: :ink|:model|:lift|:truth, dash: bool, width: float, opacity: float, draw: bool}] bands=[%{lower:, upper:, x_offset:, fill: "var(--color-model-band)"|"var(--color-lift-band)"}] vlines=[%{x:, label:}] hlines=[%{y:, label:, color:}] markers=[%{x:, y:, color:, shape: :dot|:x}] y_label= height=300 y_domain={min,max}|:auto />`
- `<.bar_figure id= items=[%{label:, value:, lower:, upper:, color:}] unit="" height=220 />`
- `<.gantt id= rows=[%{id:, start:, stop:, color:}] domain={min,max} height=140 />` (half-open windows)
- `<.matrix_grid id= matrix=[[num]] blocks=[%{from:, size:, label:}] />`
- `<.histogram id= values=[float] bins=24 vlines=[%{x:, label:, color:}] height=170 />`
...
- **Short (0.5–3 s)**: scalar MCMC (`GibbsSampler.sample`,
  ...
  ≤150 points. → `start_async` + spinner, wrapped in
...
- `Operational.run(prepared, obs, spots, return: :lists)` result:
  ...
  `summary.relative_effect.{mean,sd,lower,upper}` (fractions — ×100 for %),
